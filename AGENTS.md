...
Validate the active profile and module graph with:

```bash
nix build .#nixosConfigurations.amd-hybrid.config.system.build.toplevel
```

If you intentionally want to evaluate every exported `nixosConfiguration`, run `nix flake check` after confirming the unused profiles are expected to evaluate cleanly. For routine work, prefer the targeted build for the profile you are changing.
...
## Testing Guidelines
There is no dedicated `tests/` tree in this repository. Treat evaluation and build success as the primary regression check: run `nix build .#nixosConfigurations.<profile>.config.system.build.toplevel` for the profile you changed. Only use `nix flake check` when you want to validate every exported configuration and all of them are expected to pass. For host-specific edits, verify the matching `hosts/<hostname>/variables.nix` values still evaluate cleanly.
